...
class ImagePreprocessor:
    ...
    def detect_green_region(self, image):
        """
        Deteksi area hijau pada biji kopi untuk identifikasi biji belum matang.
        
        Args:
            image: Input BGR image
            
        Returns:
            tuple: (green_mask, green_percentage, debug_image)
        """
        # Convert ke HSV
        hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
        
        # Define range untuk warna hijau di HSV
        # Hue: 35-85 (hijau-kuning sampai hijau)
        # Saturation: 40-255 (cukup saturated)
        # Value: 40-220 (tidak terlalu gelap/terang)
        lower_green = np.array([35, 40, 40])
        upper_green = np.array([85, 255, 220])
        
        # Create mask untuk area hijau
        ...
        green_percentage = float(np.sum(green_mask > 0) / green_mask.size)
        
        # Create debug visualization
        debug_image = image.copy()
        debug_image[green_mask > 0] = [0, 255, 0]  # Highlight green areas
        
        return green_mask, green_percentage, debug_image
